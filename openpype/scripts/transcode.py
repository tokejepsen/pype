...
-arguments "-c:v prores_ks -profile:v 3 -pix_fmt yuv422p10le -qscale:v 5 /
-codec:a pcm_s16le" -output path/to/output/folder

transcode.py -input input.txt -name name -arguments "-c copy" /
...
from openpype.lib import (
    get_ffmpeg_tool_path, get_oiio_tools_path, run_subprocess
)
...
def main(input_path="",
    ...
    output_path = os.path.join(
        output_path, name + "." + output_extension
    ).replace("//", "/")
    ...
    ffmpeg_args = [get_ffmpeg_tool_path(), "-y"]
    ...
    if first_file.endswith(".exr"):
        ...
        output = run_subprocess(oiiotool_path, "--info", "-v", first_file)
    ...
    print("Running: {}".format(" ".join(args)))
    run_subprocess(*args, cwd=os.path.dirname(first_file))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-arguments", action="store", dest="arguments")
    parser.add_argument(
        "-output_extension", action="store", dest="output_extension"
    )
