...
class ProjectionLoader(api.Loader):
    ...
    def setup_nodes(self, json_path, context, control_node=None):
        ...
        camera_context = {}
        if "cameraVersionId" in json_data:
            version = io.find_one(
                {"_id": ObjectId(json_data["cameraVersionId"])}
            )
        ...
        for data in json_data["assignments"]:
            ...
            if data["attribute"] in rgb_attributes:
                material_data["rgb"] = data.get("versionId")

            if data["attribute"] in alpha_attributes:
                material_data["alpha"] = data.get("versionId")
            ...
            material_data["projection"] = data["projection"]
            material_data["color"] = data.get("color")
        ...
        for material, data in materials_data.items():
            ...
            if data["rgb"]:
                ...
                image_node["auto_alpha"].setValue(True)
                all_nodes.append(image_node)

            # Load color.
            if data["color"]:
                constant_node = nuke.createNode(
                    "Constant",
                    "color {{{} {} {} {}}}".format(
                        data["color"][0],
                        data["color"][1],
                ...
                all_nodes.append(constant_node)

            # Camera scale transform.
            if data["projection"]:
                camera_scale_node = nuke.createNode(
                    "Transform",
                    "name camera_scale scale {}".format(
                        json_data.get("cameraScale", 1.0)
                    )
                )
                all_nodes.append(camera_scale_node)

            # Premult.
            if data["rgb"]:
                premult_node = nuke.createNode("Premult")
                premult_nodes.append(premult_node)
                all_nodes.append(premult_node)
            ...
            if data["rgb"]:
                ...
                premult_node.setInput(0, image_node)
                premult_node.setXYpos(
                    image_node.xpos(), image_node.ypos() + self.node_padding
                )
                last_node = premult_node

            if data["color"]:
                if last_node:
                    constant_node.setXYpos(
                        last_node.xpos() + (2 * self.node_padding), 0
                    )
                else:
                    constant_node.setXYpos(0, 0)
                last_node = constant_node
            ...
            if data["projection"]:
                camera_scale_node.setInput(0, last_node)
                camera_scale_node.setXYpos(
                    last_node.xpos(), last_node.ypos() + self.node_padding
                )
                last_node = camera_scale_node
                ...
                camera_node.setXYpos(
                    last_node.xpos() - self.node_padding,
                    last_node.ypos()
                )
...
for node in nuke.allNodes(filter="Project3D2"):
    ...
    node[this_knob.name()].setValue(this_knob.value())
            ...
            control_node["project_on"].setValue("front")
            control_node["crop"].setValue(False)

            knob = nuke.Boolean_Knob("invert_alpha", "Invert Alpha")
            control_node.addKnob(knob)

            knob = nuke.Boolean_Knob("premult", "Premult")
            control_node.addKnob(knob)
            control_node["premult"].setValue(True)
        ...
        for node in premult_nodes:
            node["disable"].setExpression(
                "!{}.premult".format(control_node.name())
            )
