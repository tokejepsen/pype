...
class ExtractSequence(pyblish.api.Extractor):
    ...
    def process(self, instance):
        ...
        # Skip local extraction for farm instances
        if "render.farm" in instance.data.get("families", []):
            self.log.debug("Skipping local extraction — instance targets farm.")
            return
        ...
        # ------------------------------------------------------------------
        self.log.info(
            "  RENDERED file count: {}".format(
                len(output_filepaths_by_frame_idx)
            )
        )
        self.log.info(
            "  REPRESENTATION file count: {}".format(
                1 if single_file else len(repre_files)
            )
        )
        self.log.info(
            "  REPRESENTATION frameStart={} frameEnd={}".format(
                new_repre.get("frameStart"), new_repre.get("frameEnd")
            )
        )
        # ------------------------------------------------------------------
